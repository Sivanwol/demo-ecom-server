...
class RequestStoreHourSchema(Schema):
    location_id = fields.Int(missing=None, allow_none=True, validate=validate.Length(max=7, min=1, error='field location_id not valid'))
    day = fields.Int(required=True, validate=validate.Length(max=7, min=1, error='field day not valid'))
    from_time = fields.Int(required=True, validate=validate.Length(max=23, min=0, error='field from_time not valid'))
    to_time = fields.Int(required=True, validate=validate.Length(max=23, min=0, error='field to_time not valid'))
